...
{foreach from=$aCarSelectModifGroup key=sKey item=aCarSelectModif}
    ...
    {/if}
		{foreach from=$aCarSelectModif item=aModif}
			<option value="/?action=car_select{include file='car_select/xajax_link.tpl'}&year={$sCarSelectedYear}&car_select%5Bbrand%5D={$sCarSelectedBrand}&car_select%5Bmodel%5D={$sCarSelectedModel}&body={$sCarSelectedBody}&volume={$sCarSelectedVolume}&modification={$aModif.id}">{$aModif.name}</option>
		{/foreach}
	{if $aCarSelectModifGroup|@count >1 }
	   	</optgroup>
	{/if}
