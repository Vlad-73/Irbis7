...
<td>
   ...
   <br>
    {if $aRow.id_delivery_type=='6'}
	    <div class="order-num">{$oLanguage->GetMessage('np_city')}</div>
	   	{$aRow.np_city}<br>
	   	{if $aRow.np_sklad}<div class="order-num">{$oLanguage->GetMessage('np_warehouse')}</div>
	   	{$aRow.np_sklad}<br>{else}<div class="order-num">{$oLanguage->GetMessage('np_address')}</div>
	   	{$aRow.np_address}<br>{/if}
    {/if}
    <br>
