...
<td>{$aRow.make}</td>
<td>{$aRow.pref}</td>
<td><a href='http://{$smarty.server.SERVER_NAME}/?action=catalog_part_info_view&code={$aRow.code}&id_brand={$aRow.id_sup}&item_code={$aRow.item_code}&id_provider={$aRow.id_provider}&return={$sReturn|escape:"url"}'>{$aRow.code}</a></td>
<td>{$aRow.pic}</td>
