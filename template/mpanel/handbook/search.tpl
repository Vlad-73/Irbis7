            ...
            <form id="filter_form" name="filter_form" role="form" action="javascript:void(null)" onsubmit="submit_form(this)">
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm">
                            <div class="form-group">
                                <label>{$oLanguage->GetdMessage('table')}:</label>
                                <input type=text name=search[table_] value="{$aSearch.table_|escape}" maxlength=50 class="form-control btn-sm">
                            </div>

                        </div>
                        <div class="col-sm">
                            <div class="form-group">
                                <label>{$oLanguage->getDMessage('Order')}:</label>
                                <input type=text name=search[number] value="{$aSearch.number|escape}" maxlength=50 class="form-control btn-sm">
                            </div>
                        ...
                        <div class="col-sm">
                            <div class="form-group">
                                ...
                                <select name="search[is_collapsed]" class="form-control btn-sm">
                                    <option value='1' {if $aSearch.is_collapsed=='1' } selected {/if}>{$oLanguage->getDMessage('Yes')}</option>
                                    <option value='0' {if $aSearch.is_collapsed=='0' } selected {/if}>{$oLanguage->getDMessage('No')}</option>
                                    <option value='' {if $aSearch.is_collapsed=='' } selected {/if}>{$oLanguage->getDMessage('Ignore')}</option>
                                </select>
