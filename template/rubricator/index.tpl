	...
	<div class="at-index-cat-thumb"
	    ...
	    <div class="name">
	    	{if $aSubMenuItems.is_price_group}
	    		<a class="catagory_parts" href="/select/{$aSubMenuItems.url}">{$aSubMenuItems.name}</a>
	    	{else}
	    	    <a class="catagory_parts" href="/rubricator/{$aSubMenuItems.url}/{$sAutoPreSelected}">{$aSubMenuItems.name}</a>
	    	{/if}
	    </div>
