...
// 		    if (!Base::$aRequest['data']['id_model_detail'] || strpos(Base::$aRequest['data']['id_model_detail'], ",")!==false ) {
		    if ((!Base::$aRequest['data']['id_model_detail'] || strpos(Base::$aRequest['data']['id_model_detail'], ",")!==false ) && !Base::$aRequest['data']['id_model']) {
            		...
            		inner join cat_tree_link ct on ct.id_cat_part=cp.id and ct.id_tree IN ('".implode("','", Base::$aRequest['parts'])."') and ct.id_group in ('".implode("','",Base::$aRequest['groups'])."')
            		";
		            if(Base::$aRequest['data']['id_model_detail']) {
		                $sSqlSiteParts.="inner join cat_model_type_link cm on cm.id_cat_part=cp.id and cm.id_cat_model_type in (".Base::$aRequest['data']['id_model_detail'].")";
		            }
